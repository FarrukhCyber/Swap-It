...
mysql = create_db()
entryb = Blueprint('entry', __name__)


@entryb.route('/entry', methods=['GET', 'POST'])
def entry():
# done with -> student | instructor | timeslot | section | course | teaches | admin | takes
    ...
    roll_number = 23100105
    # adds 5k student entries 
    for i in range(500): # adds 5k student entries
        value[0] = roll_number
        value[1] = names.get_full_name()
        value[2] = roll_number
        value[3] = str(roll_number) + "@lums.edu.pk"
        value[4] = random.choice(credit)
        value[5] = random.choice(dept)
    ...
    course_id = 400
    t_inst = 7
    for i in range(50): # adds 100 courses and adjusts section + teaches + timeslot 
    ...
    admin_id = 10
    for i in range(5): # adds 10 admin entries
    ...
    ts_id = 23100115 
    ts_cid = 100
    for i in range(1000): # takes 
        ts_sec = random.choice(section)
        ts_dept = random.choice(dept)
        ts_course = ts_dept + str(ts_cid)
        ts_val = [ts_id,ts_course,ts_sec,"A"]
        cur.execute("Insert into takes values(%s,%s,%s,%s)", ts_val)
        cur.connection.commit()
        ts_id+=1
        ts_cid+=1
